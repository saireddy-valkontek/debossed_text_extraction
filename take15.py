...
def main():
    ...
    parser.add_argument("--image", type=str, help="Path to image file for detection")  # Image input
    ...
    img = None
    if args.image:
        img = cv2.imread(args.image)  # Load image from file if provided
        if img is None:
            logging.error("[ERROR] Could not read the image from the file.")
    else:
