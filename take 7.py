...
def process_image(image_path, yolo_model, crnn_model):
    try:
        ...
        for idx in keep:
            ...
            label_bg_top_left = (x1, y1 - th - 10 if y1 - th - 10 > 0 else y1 + th + 10)
            label_bg_bottom_right = (x1 + tw, y1 if y1 - th - 10 > 0 else y1 + 2)
            cv2.rectangle(img, label_bg_top_left, label_bg_bottom_right, (0, 255, 0), -1)

            # Put text
            text_pos = (x1, y1 - 5 if y1 - th - 10 > 0 else y1 + th + 5)
            cv2.putText(img, label, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 2)

        print("[INFO] Saving output image with labels...")
